        ...
        <tbody>
            ...
            <tr class="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                ...
                <td class="px-5 py-3 text-right">
                    <button onclick="editClass({{ $class->id }}, '{{ $class->name }}', '{{ $class->code }}', {{ $class->level }})" class="text-emerald-600 hover:text-emerald-700 text-xs font-medium">Edit</button>
                    <form action="{{ route('classes.destroy', $class) }}" method="POST" class="inline" onsubmit="return confirm('Delete this class?')">
                        @csrf @method('DELETE')
            ...
            @empty
            <tr><td colspan="4" class="px-5 py-8 text-center text-gray-400">No classes yet.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
...
<div id="modal-create" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/40">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
        <h3 class="text-lg font-bold text-gray-900 mb-4">Add Class</h3>

...
<div class="mb-4 flex items-center justify-between flex-wrap gap-3">
    <div class="flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-xs text-gray-400">{{ $subject->name }} · Max {{ $subject->max_marks }} · {{ $assignment->district->name }}</p>
        </div>
    </div>
    <div class="flex items-center gap-3">
...
<div class="w-full bg-gray-100 rounded-full h-2 mb-4">
    <div id="progress-bar" class="bg-emerald-500 h-2 rounded-full transition-all duration-300" style="width: {{ $candidates->count() > 0 ? ($marks->whereNotNull('mark')->count()/$candidates->count()*100) : 0 }}%"></div>
</div>
...
(function() {
    ...
    function autoSave(input) {
        ...
        fetch(AUTO_SAVE_URL, {
            ...
            body: JSON.stringify({
                ...
                mark: markValue === '' ? null : parseFloat(markValue),
            })
        })
        .then(r => r.json())
        .then(data => {
    ...
    inputs.forEach((input, idx) => {
        ...
        input.addEventListener('blur', function() {
            if (this.value.trim() !== '' || this.dataset.hadValue === 'true') {
                autoSave(this);
            }
        });

        input.addEventListener('focus', function() {
            this.dataset.hadValue = this.value.trim() !== '' ? 'true' : 'false';
            this.select();
        });

        input.addEventListener('input', function() {
            const val = parseFloat(this.value);
            if (this.value.trim() !== '' && val > MAX_MARKS) {
                this.classList.add('border-red-500', 'ring-2', 'ring-red-100');
            } else {
